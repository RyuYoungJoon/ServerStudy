...
int main()
{
	...
	// WSAAsyncSelect 모델 = 소켓 이벤트를 윈도우 메시지 형태로 처리 (일반 윈도우 메시지랑 같이 처리하니 성능이 아리달쏭)
	// WSAEventSelect 모델
	// - 장점) 비교적 뛰어난 성능
	// - 단점) 64개 제한
	// Overlapped (이벤트 기반)
	// - 장점) 성능이 좋다.
	// - 단점) 64개 제한
	// Overlapped (콜백 기반) apc큐
	// - 장점) 모든 비동기 소켓 함수에서 사용 가능하진 않다. (accept). 빈번한 Alertable WAIT으로 인한 성능 저하.
	// IOCP
